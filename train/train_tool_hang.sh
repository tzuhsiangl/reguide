#!/bin/bash
# Train the base diffusion policy for the `tool_hang` task.
#
#   conda activate reguide
#   bash train/train_tool_hang.sh
...
"$PYTHON" train.py \
  ...
  hydra.run.dir="${OUTPUT_ROOT}/${NUM_DEMOS}/${CATEGORY}/\${now:%Y.%m.%d}/\${now:%H.%M.%S}" \
  logging.name="\${now:%Y.%m.%d}_\${now:%H.%M.%S}_tool_hang_image_${NUM_DEMOS}_${CATEGORY}" \
