#!/bin/bash
# Train the base diffusion policy for the `transport` task.
#
#   conda activate reguide
#   bash train/train_transport.sh
...
"$PYTHON" train.py \
  ...
  logging.name="\${now:%Y.%m.%d}_\${now:%H.%M.%S}_transport_image_${NUM_DEMOS}_${CATEGORY}_final" \
