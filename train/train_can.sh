#!/bin/bash
# Train the base diffusion policy for the `can` task.
#
#   conda activate reguide
#   bash train/train_can.sh
...
"$PYTHON" train.py \
  ...
  logging.name="\${now:%Y.%m.%d}_\${now:%H.%M.%S}_train_diffusion_unet_hybrid_can_image_${NUM_DEMOS}_${CATEGORY}" \
