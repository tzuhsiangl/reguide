#!/bin/bash
# ReGuide Fine-Tune (FT) for a LIBERO task: continue training the base policy on
# (expert demos + guided success rollouts).
#
#   conda activate reguide          # LIBERO must also be installed / importable
#   bash train_reguide-ft/train_libero_ft.sh
...
# ---- experiment knobs ----
TASK="libero"
NUM_DEMOS="LIVING_ROOM_SCENE6"        # LIBERO scene/task name
NUM_ROLLOUT=50
CATEGORY="rollout_success_first_${NUM_ROLLOUT}"
EXPERT_SAMPLE_WEIGHT=0.2
ROLLOUT_REUSE_RATE=2

# Exact language goal for this scene's task (must match an entry in language_goals_list verbatim).
ROLLOUT_LANGUAGE_GOAL="LIVING_ROOM_SCENE6_put_the_white_mug_on_the_plate_and_put_the_chocolate_pudding_to_the_right_of_the_plate"

# ---- datasets, init checkpoint, output ----
EXPERT_DATASET="path/to/libero/training_data"   # directory of per-task expert hdf5 files
...
echo " ROLLOUT_LANGUAGE_GOAL=$ROLLOUT_LANGUAGE_GOAL"
...
"$PYTHON" train.py \
  ...
  task.dataset.rollout_language_goal="$ROLLOUT_LANGUAGE_GOAL" \
  task.env_runner.dataset_path="$EXPERT_DATASET" \
  task.dataset_path="$EXPERT_DATASET" \
  hydra.run.dir="${OUTPUT_ROOT}/${NUM_DEMOS}/${CATEGORY}_share_${EXPERT_SAMPLE_WEIGHT}/\${now:%Y.%m.%d}/\${now:%H.%M.%S}" \
  logging.name="\${now:%Y.%m.%d}_\${now:%H.%M.%S}_train_diffusion_unet_hybrid_${NUM_DEMOS}_${CATEGORY}_share_${EXPERT_SAMPLE_WEIGHT}" \
  ...
  training.max_train_steps=30000 \
