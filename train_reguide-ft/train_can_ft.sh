#!/bin/bash
# ReGuide Fine-Tune (FT) for the `can` task: continue training the base policy on
# (expert demos + guided success rollouts).
#
#   conda activate reguide
#   bash train_reguide-ft/train_can_ft.sh
...
RUN_NAME="\${now:%Y.%m.%d}_\${now:%H.%M.%S}_train_diffusion_unet_hybrid_can_image_${NUM_DEMOS}_${CATEGORY}_expert_ratio_${EXPERT_SAMPLE_WEIGHT}"
