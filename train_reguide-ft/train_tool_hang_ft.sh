#!/bin/bash
# ReGuide Fine-Tune (FT) for the `tool_hang` task: continue training the base policy on
# (expert demos + guided success rollouts).
#
#   conda activate reguide
#   bash train_reguide-ft/train_tool_hang_ft.sh
...
# ---- experiment knobs ----
TASK="tool_hang"
NUM_DEMOS="80_demos_20"
NUM_ROLLOUT=60
CATEGORY="rollouts_first_${NUM_ROLLOUT}"
...
RUN_NAME="\${now:%Y.%m.%d}_\${now:%H.%M.%S}_train_diffusion_unet_hybrid_tool_hang_image_${NUM_DEMOS}_${CATEGORY}_expert_ratio_${EXPERT_SAMPLE_WEIGHT}"
