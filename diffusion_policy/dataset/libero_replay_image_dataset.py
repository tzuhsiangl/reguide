...
class LiberoReplayImageDataset(BaseImageDataset):
    def __init__(
        ...
        data_aug=False,
        language_goal=None,  # NEW: single-task goal override (used for rollout buffers)
    ):
        ...
        if use_cache:
            ...
            if language_emb_model == "clip":
                # Fold the override into the cache name so an overridden buffer gets
                # its own cache and never silently reuses a differently-labeled one.
                suffix = ""
                if language_goal is not None:
                    safe = str(language_goal).strip().replace(" ", "_").replace("/", "-")
                    suffix = f"_lg-{safe}"
                cache_zarr_path = dataset_path + suffix + "_clip_both_views.zarr.zip"
            else:
                ...
                if not os.path.exists(cache_zarr_path):
                    try:
                        ...
                        replay_buffer = _convert_robomimic_to_replay(
                            ...
                            language_goal_override=language_goal,  # NEW
                        )
                        print("Saving cache to disk.")
                        with zarr.ZipStore(cache_zarr_path) as zip_store:
                            replay_buffer.save_to_store(store=zip_store)
                    except Exception as e:
                        print("[REAL CACHE CREATION ERROR]", repr(e))
                        if os.path.isdir(cache_zarr_path):
                            shutil.rmtree(cache_zarr_path)
                        elif os.path.isfile(cache_zarr_path):
...
def _convert_robomimic_to_replay(
    ...
    language_emb_model=None,
    language_goal_override=None,  # NEW: when set, all demos in this path use this goal
):
    ...
    if os.path.isfile(dataset_path):
        ...
        dataset_paths = sorted(glob.glob(os.path.join(dataset_path, "*_demo.hdf5")))
    else:
        raise FileNotFoundError(f"dataset_path does not exist: {dataset_path}")

    print("Resolved dataset_paths:", dataset_paths)

    def _normalize_goal(s: str) -> str:
        # Existing convention is space-separated; accept either form.
        return " ".join(s.replace(" ", "_").split("_"))

    for dataset_path_each in dataset_paths:
        # Filename-based default preserves original behavior for expert files.
        default_language_goal = _normalize_goal(
            os.path.basename(dataset_path_each)[:-len("_demo.hdf5")]
        )

        print(f"Loading {dataset_path_each}")
        file = h5py.File(dataset_path_each, "r")
        ...
        for i in range(len(demos)):
            ...
            # Priority: explicit override > per-demo attr > filename stem.
            if language_goal_override is not None:
                this_goal = _normalize_goal(str(language_goal_override))
            elif "language_goal" in demo.attrs:
                lg_raw = demo.attrs["language_goal"]
                if isinstance(lg_raw, bytes):
                    lg_raw = lg_raw.decode("utf-8")
                this_goal = _normalize_goal(str(lg_raw))
            else:
                this_goal = default_language_goal

            assert this_goal in language_goals_list, (
                f"Language goal '{this_goal}' not in language_goals_list "
                f"(source: {dataset_path_each}, demo_{i})"
            )

            demos_all[f"demo_{count}"] = demo
            language_all[f"demo_{count}"] = this_goal
            count += 1
