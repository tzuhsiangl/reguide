...
def get_robomimic_model_file(env_name):
    ...
    if 'Libero' in env_name:
        ...
        with open(model_path, 'r') as f:
            model_file = f.read()
    elif "PickPlaceCan" in env_name:
        model_path = os.path.join(xml_base_path, "can_model_file_1.4.xml")
        with open(model_path, 'r') as f:
            model_file = f.read()
    elif "Lift" in env_name:
        model_path = os.path.join(xml_base_path, "lift_model_file_1.4.xml")
        with open(model_path, 'r') as f:
            model_file = f.read()
    return model_file
